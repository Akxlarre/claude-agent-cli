...
## linkedSignal en Facades (Two-Way Binding)

Patrón para formularios donde el estado local está vinculado a un recurso del Facade.
Cuando el recurso cambia, el form se resetea automáticamente.

```typescript
// En el Smart Component (features/)
readonly formName = linkedSignal(() => this.facade.selectedUser()?.name ?? '');
readonly formEmail = linkedSignal(() => this.facade.selectedUser()?.email ?? '');

readonly isValid = computed(() =>
  this.formName().trim().length > 0 && validateEmail(this.formEmail())
);

async save(): Promise<void> {
  await this.facade.updateUser({ name: this.formName(), email: this.formEmail() });
}
```

El linkedSignal permite que el usuario edite los campos (es writable), pero si
`facade.selectedUser()` cambia (ej: navegación a otro usuario), los campos se
resetean automáticamente al nuevo valor. Ideal para formularios de edición inline
y wizards multi-step.
